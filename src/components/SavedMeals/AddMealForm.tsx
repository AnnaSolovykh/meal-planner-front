...
const AddMealForm = ({ onCreateMeal, errorMessage }: AddMealFormProps) => {
  ...
  const handleIngredientKeyPress = (event: {
    key: string;
    preventDefault: () => void;
  }) => {
    ...
    >
      <div>
        ...
        <FormControl fullWidth margin="normal">
          ...
          <Select value={type} label="Type" onChange={createType}>
      ...
      <div>
        ...
        <FormControlLabel
          control={
            <Checkbox checked={isFavorite} onChange={toggleIsFavorite} />
          }
          label="Favorite"
        />
      </div>
      ...
      {errorMessage && (
        <Typography
          variant="body2"
          style={{ color: 'darkred', marginBottom: '10px', textAlign: 'left' }}
        >
          {errorMessage}
        </Typography>
      )}
