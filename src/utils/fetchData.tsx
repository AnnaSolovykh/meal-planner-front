...
export const fetchLogin = (email: string, password: string) => {
    return api.post(`http://localhost:4000/api/v1/auth/login`, 
        {
            email,
            password,
        },
...
export const fetchRegister = (name: string, email: string, password: string) => {
    return api.post(`http://localhost:4000/api/v1/auth/register`, 
        {
            name,
            email,
            password,
...
export const getMeals = (typeFilter: string, titleFilter: string, isFavoriteFilter: boolean, currentPage: number, limit: number): Promise<MealsType> => {
    ...
    return api.get(`http://localhost:4000/api/v1/meals?${query}`)
